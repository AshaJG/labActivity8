  ...
  def apply(number: Int): GameMap ={
    if(number == 0){
      ...
      new GameMap{
        ...
        startingLocation = new GridLocation(8, 10)
...
//   val  makeTiles = (tile: MapTile) => Json.toJson(Map("type" -> Json.toJson(tile.tileType), "passable" -> Json.toJson(tile.passable)))
//    val makeRow = (row: Int) => tiles(row).indices.map(makeTiles)
//    Json.toJson(tiles.indices.map(makeRow))
